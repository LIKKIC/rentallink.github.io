   ...
   var sql = "INSERT INTO ownersignup(name,username,email_ID,contact,pwd,address)VALUES('"+name+"','"+username+"','"+email_id+"','"+contact+"','"+pwd+"','"+address+"')";
    ...
    app.get('/updatepg',(req,res)=>{
      ...
      app.post('/addpg',(req,res)=>{
        ...
        var sql = "INSERT INTO pg(Name,landmark,No_Rooms,Rooms_Filled,Rooms_vac,Address,facilities,foods,Owner_ID)VALUES('"+Name+"','"+landmark+"','"+No_Rooms+"','"+Rooms_Filled+"','"+Rooms_vac+"','"+Address+"','"+facilities+"','"+foods+"','"+Owner_ID+"')";
            con.query(sql,function(err,result){
                if(err) throw err;
                alert("pg added");
                res.redirect('/ownerprofile');
